...
l = 0
m = 0
journalist.each do |contact|
    if contact =~ /[1234567890]/
        i += 1
    end
    if contact.downcase.include?("aude")    
        j += 1
    end
    if contact =~ /^@[A-Z]/
        k += 1
    ...
    if contact.include?("_")
        m += 1
    end
end

puts "il y a #{i} journalistes avec un chiffre"

#Combien de Aude ?
puts "il y a #{j} Aude dans la liste"

#Combien commencent par une majuscule ?
puts "il y a #{k} journalistes qui commencent par une majuscule"

#Combien contiennent une majuscule ?
puts "il y a #{l} journalistes qui contiennent une majuscule"

#Combien de _ dans tous les pseudos confondus ?
puts "il y a #{m} journalistes qui ont _ dans leurs pseudos"
